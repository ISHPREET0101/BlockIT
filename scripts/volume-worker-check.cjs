...
async function scan(workerPath,base,imagePath) {
  ...
  try {
    ...
    assert.equal(totals.files,9000); assert.equal(totals.bytes,63000);
    ...
    assert.equal(root.file_count,9000); assert.equal(root.size,63000);
...
(async()=>{
  ...
  const imagePath=path.join(base,'bulk.img');buildBulkImage(imagePath,9000);
  ...
  const before=[],after=[];
  try {
    for(let i=0;i<(beforePath?3:1);i++) {
    ...
    const report={fixtureFiles:9000,before,after,beforeMedianMs:before.length?median(before):null,afterMedianMs:median(after),note:'Synthetic NTFS image on this machine; not a real whole-drive speed guarantee.'};
    console.log(JSON.stringify(report,null,2));
    fs.writeFileSync(path.resolve(__dirname,'../docs/volume-worker-performance.json'),JSON.stringify(report,null,2)+'\n');
