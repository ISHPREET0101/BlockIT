...
export const TreemapView = memo(function TreemapView({ loading, depth, onDepthChange, nodes, title, settings, canReset, onOpen, onReset, onExport, onCopyPath }: TreemapProps) {
  ...
  const leaves = useMemo(() => {
    ...
    return treemap<LayoutDatum>().size([1200,560]).paddingInner(6).paddingOuter(6)
  ...
  }, [nodes,settings.useAllocatedSize,depth,minimum]);
  ...
  return <section className="panel treemap-panel">
          ...
          {leaves.map((leaf,index)=>{
            ...
            return <g key={node.id} transform={'translate('+leaf.x0+','+leaf.y0+')'} role={node.synthetic?'group':'button'}
              ...
              {settings.showLabels && width>70 && height>42 && <g clipPath={'url(#'+clip+')'} pointerEvents="none" fill={ink} fontFamily="Segoe UI, sans-serif">
                ...
                {!leaf.children && <text x="14" y="47" fontSize="12">{formatBytes(sizeOf(node),settings.unit)+(width>165?' · '+share(node):'')}</text>}
    ...
    <details className="map-block-list">
      ...
      <div>{allNodes.filter(matches).map(node=><button key={node.id} onClick={()=>activate(node)} className="map-list-item" aria-label={'Inspect '+node.name}>
        <i style={{background:colorOf(node),border:node.syntheticKind==='free'?'1px dashed #637d68':undefined}}/>
