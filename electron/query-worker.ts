...
import { configureDirectory,getSummary,queryNodes,treemapChildren,treemapTree,ancestors,closeDatabases,exportCsvFile,reconcileRemoval } from './queries';
...
parentPort?.on('message',async ({id,operation,args})=>{
  try {
    ...
    else if(operation==='treemap') result=treemapChildren(args[0],args[1]);
    else if(operation==='treemapTree') result=treemapTree(args[0],args[1],args[2]);
